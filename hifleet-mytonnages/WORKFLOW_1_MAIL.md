...
助手在用户确认信息无误后，使用 **Openclaw 允许的方式** 写入配置（例如内置写入工具、设置界面、或约定路径下的安全存储）。**不得**要求用户运行 `mkdir`/`echo`/`chmod` 或任何终端命令。

推荐落盘路径（若环境支持文件存储）：`~/.openclaw/workspace/skills/hifleet-mytonnages/config.json`，敏感字段按 OpenClaw / 插件规范保存（如单独的密钥存储），**勿在普通对话里复述用户密码**。

若当前 OpenClaw 版本仅支持「对话内临时邮箱」而无持久化，则按该版本能力降级，并在说明中如实告知用户限制。
